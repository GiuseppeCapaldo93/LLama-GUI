...
def _build_active_runtime(
    ...
    has_vision = any(
        arg.split("=", 1)[0] in {"-mm", "--mmproj", "--mmproj-url"} for arg in flat_args
    ) and not any(arg == "--no-mmproj" for arg in flat_args)
    runtime = {
...
def _request_graceful_stop(process) -> bool:
    ...
    Returns False when the signal could not be delivered (for example, on
    Windows the GUI often runs under pythonw.exe with no console, so
    CTRL_BREAK_EVENT raises "The handle is invalid"). In that case the caller
    should force-kill instead of leaving the process running.
    """
    try:
        if sys.platform == "win32":
